...
const getAllCalculators = async (req, res) => {
  try {
    ...
    let list = [...CALCULATOR_DATA];

    if (category) {
      list = list.filter(c => c.category.toLowerCase() === category.toLowerCase() || c.category.toLowerCase().replace(/\s+/g, '-') === category.toLowerCase());
    }
...
const getCalculatorBySlug = async (req, res) => {
  try {
    const calc = CALCULATOR_DATA.find(c => c.slug === req.params.slug);
    if (!calc) {
...
module.exports = { getAllCalculators, getCalculatorBySlug, getCategories, CALCULATOR_DATA, CATEGORIES_DATA };
